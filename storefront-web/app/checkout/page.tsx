...
export default async function CheckoutPage({ searchParams }: CheckoutPageProps) {
  ...
  const productValue = Array.isArray(rawProduct) ? rawProduct[0] || "" : rawProduct || "";
  const parsedProductId = Number.parseInt(String(productValue || "").trim(), 10);
  const initialCheckoutProductId = Number.isFinite(parsedProductId) && parsedProductId > 0 ? parsedProductId : null;
  ...
  return (
    <StorefrontPage
      ...
      pageMode="checkout"
      initialCheckoutProductId={initialCheckoutProductId}
    />
  );
}
